...
public class ReportContoller {
    ...
    public Result getMemberSexReport(){
        try {
             ...
             * 返回Map<String,Object>
             map集合的key：                       map集合的value：
             setmealNames                          List<String>   -->['男','女']
             setmealCount                          List<Map<String,Object>> -->[
             {value: 335, name: '男'},
             {value: 310, name: '女'}
             ]
             */
            Map<String,Object> map = new HashMap<>();//给前端的数据值赋值
            // 组织List<Map>
            List<Map> memberSexCount = memberService.findSexCountByMemberSex();
            // 组织List<String>  给前端的名称分类赋值
            List<String> memberSexs = new ArrayList<>();
            // 遍历setmealCount
            if(memberSexCount!=null && memberSexCount.size()>0){
                for (Map memberSexMap : memberSexCount) {
                    String name = (String)memberSexMap.get("name");
                    memberSexs.add(name);
                }
            }
    ...
    public Result getMemberAgeReport(){
        try {
             ...
             * 返回Map<String,Object>
             map集合的key：                       map集合的value：
             setmealNames                          List<String>   -->['年龄段','年龄段']
             setmealCount                          List<Map<String,Object>> -->[
             {value: 335, name: '年龄段'},
             {value: 310, name: '年龄段'}
             ]
            ...
            Map<String,Object> map = new HashMap<>();//给前端的数据值赋值
            // 组织List<Map>
            List<Map> memberAgeCount = memberService.findAgeCountByMemberAge();
            // 组织List<String>  给前端的名称分类赋值
            List<String> memberAges = new ArrayList<>();
            // 遍历setmealCount
            if(memberAgeCount!=null && memberAgeCount.size()>0){
                for (Map memberAgeMap : memberAgeCount) {
                    String name = (String)memberAgeMap.get("name");
                    memberAges.add(name);
                }
            }
